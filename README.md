...
- **Namespace-level protection** — new `protected_namespaces` (glob patterns) gate state-altering commands by namespace, composing with context protection. `--all-namespaces`/`-A` is gated when any namespace is protected.
